...
# 상단 배너 이미지 추가
st.image("https://images.unsplash.com/photo-1590301157890-4810ed352733?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80", use_container_width=True)
...
if uploaded_file is not None:
    try:
        ...
        df['정산연도'] = df['날짜'].apply(lambda x: f"{get_custom_month(x)[0]}년")

        # 사이드바 디자인 개선
        with st.sidebar:
            st.image("https://cdn.pixabay.com/photo/2017/08/08/09/44/food-2610863_1280.jpg", caption="맛있는 정산 시간 🍱")
            st.divider()
            mode = st.radio("보고서 선택", ["월별 상세 분석 (8일~7일)", "연도별 총 매출"])
